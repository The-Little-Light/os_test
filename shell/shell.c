...
#define cmd_len MAX_PATH_LEN // 最大支持键入 MAX_PATH_LEN 个字符的命令行输入
...
/* 分析字符串 cmd_str 中以 token 为分隔符的单词，
将各单词的指针存入 argv 数组 */
